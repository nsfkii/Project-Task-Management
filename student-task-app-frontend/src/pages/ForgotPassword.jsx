...
export default function ForgotPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            Swal.fire({ icon: 'success', title: 'OTP Terkirim', text: 'Cek email Anda untuk kode OTP.' });
            navigate('/verify-otp', { state: { email } });
        } catch (error) {
            Swal.fire({
                icon: 'error',
                title: 'Gagal',
                text: error?.response?.data?.message || 'Tidak dapat mengirim OTP.',
            });
        } finally {
            setLoading(false);
        }
        ...
        <div className="min-h-screen flex items-center justify-center px-4 bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800">
            <div className="w-full max-w-md p-8 bg-white/95 dark:bg-slate-800/95 border border-slate-100 dark:border-slate-700 rounded-2xl shadow-xl">
                <div className="flex flex-col items-center mb-6">
                    ...
                    <h1 className="mt-3 text-2xl font-bold text-gray-900 dark:text-white">Lupa Password</h1>
                </div>
